...
const Settings: React.FC<SettingsProps> = ({ promotion, isEdit = false }) => {
  const { register, control, isDynamic, hasExpiryDate, handleConfigurationChanged } =
    useDiscountForm();
        ...
        {render && (
          <>
            <Accordion.Item
              ...
              value="starts_at"
              customTrigger={<Switch checked={openItems.includes('starts_at')} />}
            >
              <div
                className={clsx('flex items-center gap-xsmall', marginTransition, {
                  'mt-4': openItems.includes('starts_at'),
                })}
              >
                <Controller
                  name="starts_at"
            ...
            <Accordion.Item
              ...
              value="ends_at"
              customTrigger={<Switch checked={openItems.includes('ends_at')} />}
            >
              <div
                className={clsx('flex items-center gap-xsmall', marginTransition, {
                  'mt-4': openItems.includes('ends_at'),
                })}
              >
                <Controller
                  name="ends_at"
            ...
            <Accordion.Item
              ...
              value="usage_limit"
              customTrigger={<Switch checked={openItems.includes('usage_limit')} />}
            >
              <div
                className={clsx(marginTransition, {
                  'mt-4': openItems.includes('usage_limit'),
                })}
              >
                <InputField
            ...
            {isDynamic && (
              <Accordion.Item
                ...
                customTrigger={<Switch checked={openItems.includes('valid_duration')} />}
              >
                <div
                  className={clsx(marginTransition, {
                    'mt-4': openItems.includes('valid_duration'),
                  })}
                >
                  <Controller
                    name="valid_duration"
