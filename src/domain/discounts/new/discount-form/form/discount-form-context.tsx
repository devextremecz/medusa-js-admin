...
export const DiscountFormProvider = ({ children }: DiscountFormProviderProps) => {
  ...
  const handleConfigurationChanged = (values) => {
    if (values.includes('ends_at') && !hasExpiryDate) {
      setHasExpiryDate(true);
    } else if (!values.includes('ends_at') && hasExpiryDate) {
      setHasExpiryDate(false);
    }

    if (!values.includes('starts_at') && hasStartDate) {
      setHasStartDate(false);
    } else if (values.includes('starts_at') && !hasStartDate) {
      setHasStartDate(true);
    }

    // usage_limit
    if (!values.includes('usage_limit') && usageLimit) {
    ...
    } else if (values.includes('usage_limit') && usageLimit) {
      methods.setValue('usage_limit', prevUsageLimit);
    }

    // valid duration
    if (!values.includes('valid_duration') && validDuration !== '') {
      setPrevValidDuration(validDuration);
      // debounce the setValue call to not flash an empty field when collapsing the accordion
      setTimeout(() => {
        methods.setValue('valid_duration', '');
      }, 300);
    } else if (values.includes('valid_duration') && validDuration === '') {
