...
const EditVariantModal = ({ onClose, product, variant, isDuplicate = false }: Props) => {
  ...
  const { onUpdateVariant, onAddVariant, addingVariant, updatingVariant } = useEditProductActions(
    product.id
  );
    ...
    <Modal handleClose={handleClose}>
      <Modal.Header handleClose={handleClose}>
        <h1 className="inter-xlarge-semibold">
          Edit Variant
          {variant.title && (
            <span className="text-grey-50 inter-xlarge-regular"> ({variant.title})</span>
          )}
        </h1>
      </Modal.Header>
