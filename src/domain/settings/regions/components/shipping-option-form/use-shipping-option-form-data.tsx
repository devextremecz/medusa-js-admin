...
export const useShippingOptionFormData = (regionId: string, isReturn = false) => {
  ...
  const fulfillmentOptions: Option[] = useMemo(() => {
    ...
    return fulfillment_options.reduce((acc, current, index) => {
  ...
  const getRequirementsData = (data: ShippingOptionFormType) => {
    return Object.entries(data.requirements).reduce((acc, [key, value]) => {
    ...
    }, [] as Array<{ type: string; amount: number; id?: string }>);
  };
