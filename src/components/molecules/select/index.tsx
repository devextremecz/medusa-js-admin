...
type MultiSelectProps = InputHeaderProps & {
  ...
  labelledBy?: string;
  options: Array<{ label: string; value: string | null; disabled?: boolean }>;
  value: Array<{ label: string; value: string }> | { label: string; value: string } | null;
...
const GetSelect = React.forwardRef(
  ({ isCreatable, searchBackend, onCreateOption, handleClose, ...props }, ref) => {
    if (isCreatable) {
      ...
      return (
        <AsyncPrimitive ref={ref} defaultOptions={true} loadOptions={searchBackend} {...props} />
      );
    }
